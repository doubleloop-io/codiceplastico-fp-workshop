...
object Round7 {
  ...
  def checkField(filed: String): Rule[FormData, String] =
    value => value.get(filed).fold[Result[String]](Fail(List(MissingField)))(v => Success(v))

  val checkName: Rule[FormData, String] =
    value => checkField("name")(value).flatMap(checkNotEmpty(_))

  val checkAge: Rule[FormData, Int] =
    value => checkField("age")(value).flatMap(checkNumber(_))

  val checkPerson: Rule[FormData, Person] =
    value => (checkName(value), checkAge(value)).mapN(Person.apply)
}
